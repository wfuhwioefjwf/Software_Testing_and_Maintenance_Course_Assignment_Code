...
class ExpConfig(mltk.Config):
    ...
    def _model_post_checker(self, v: 'ExpConfig'):
        ...
        # Online-Boutique is closer to ASD/SMD application-server monitoring data.
        # The original InterFusion MTSAD architecture is designed with W=100.
        if v.dataset == 'online_boutique':
            v.model.x_dim = 16
            v.model.window_length = 100
            v.model.z_dim = 3
...
def main(exp: mltk.Experiment[ExpConfig], config: ExpConfig):
    ...
    random.seed(np.random.randint(0x7fffffff))
    tf.set_random_seed(np.random.randint(0x7fffffff))
    np.random.seed(np.random.randint(0x7fffffff))
