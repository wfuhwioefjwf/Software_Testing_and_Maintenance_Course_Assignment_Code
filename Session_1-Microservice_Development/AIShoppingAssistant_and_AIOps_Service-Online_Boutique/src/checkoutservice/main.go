...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	promoQuote, err := cs.quotePromotion(ctx, req.UserId, &subtotal, prep.shippingCostLocalized)
	if err != nil {
		return nil, status.Errorf(codes.Unavailable, "promotion service unavailable: %+v", err)
	}
	promoDiscount := promoQuote.Discount.proto()
	if promoQuote.Valid && !money.IsZero(*promoDiscount) {
		total = *promoQuote.FinalTotal.proto()
		log.Infof("promotion applied (code=%q discount=%d.%09d total=%d.%09d)",
			promoQuote.Code,
			promoDiscount.GetUnits(),
			promoDiscount.GetNanos(),
			total.GetUnits(),
			total.GetNanos())
	} else if promoQuote.Code != "" {
		log.Infof("promotion rejected (code=%q message=%q)", promoQuote.Code, promoQuote.Message)
	}
