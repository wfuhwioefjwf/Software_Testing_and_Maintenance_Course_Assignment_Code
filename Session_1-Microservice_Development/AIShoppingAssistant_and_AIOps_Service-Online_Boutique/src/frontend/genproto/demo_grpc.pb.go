...
const (
	ShoppingAssistantService_Chat_FullMethodName = "/hipstershop.ShoppingAssistantService/Chat"
)
...
func (c *shoppingAssistantServiceClient) Chat(ctx context.Context, in *ChatRequest, opts ...grpc.CallOption) (ShoppingAssistantService_ChatClient, error) {
	stream, err := c.cc.NewStream(ctx, &ShoppingAssistantService_ServiceDesc.Streams[0], ShoppingAssistantService_Chat_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
...
type ShoppingAssistantServiceServer interface {
	Chat(*ChatRequest, ShoppingAssistantService_ChatServer) error
	mustEmbedUnimplementedShoppingAssistantServiceServer()
}

// UnimplementedShoppingAssistantServiceServer must be embedded to have forward compatible implementations.
type UnimplementedShoppingAssistantServiceServer struct {
}

func (UnimplementedShoppingAssistantServiceServer) Chat(*ChatRequest, ShoppingAssistantService_ChatServer) error {
...
func _ShoppingAssistantService_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ChatRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ShoppingAssistantServiceServer).Chat(m, &shoppingAssistantServiceChatServer{stream})
}

type ShoppingAssistantService_ChatServer interface {
	Send(*ChatResponse) error
	grpc.ServerStream
}

type shoppingAssistantServiceChatServer struct {
	grpc.ServerStream
}

func (x *shoppingAssistantServiceChatServer) Send(m *ChatResponse) error {
...
var ShoppingAssistantService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Chat",
			Handler:       _ShoppingAssistantService_Chat_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "demo.proto",
