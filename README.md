...
在异常数据采集与论文复现方面，本项目基于 Online Boutique 监控数据和故障注入数据，完成了多篇异常检测与故障诊断论文复现，包括 `CATCH`、`KPIRoot`、`KAN-AD`、`DiMER`、`SRCNN` 和 `InterFusion`，覆盖异常检测、根因定位、多变量时序分析等方向，对应作业要求中的异常数据采集、算法复现与效果分析内容，也满足加分项中“更多论文复现与对比”的要求。
